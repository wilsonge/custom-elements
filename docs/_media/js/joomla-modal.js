(() => {
  if (!window.HTMLDialogElement) {
    // Do the polyfill
    if (!window.Joomla || !window.Joomla.getOptions('system.paths') || !window.Joomla.getOptions('system.paths').rootFull) {
      throw new Error('`Joomla-modal` requires a unique id attribute');
    }
  }
  ...
  const modal = () => {
    customElements.define('joomla-modal', class extends HTMLElement {
      ...
      open() {
        ...
        this.focusableElements = [].slice.call(
          this.querySelectorAll(this.focusableSelectors.join()),
        );
      ...
      adjustDimensions() {
        ...
        const maxModalBodyHeight = maxModalHeight -
          (headerHeight + footerHeight + modalBodyPadding);
